...
func NewCamera(b *PositionBuilder, cfg *common.Config) *Camera {
	return &Camera{
		positionBuilder: b,
...
func (c *Camera) MouseButton(w *glfw.Window, button glfw.MouseButton, action glfw.Action) {
...
func (c *Camera) MouseMove(x, y float64) {
	if c.rotation {
...
func (c *Camera) updateView() {
	...
	c.positionBuilder.UpdateView(view)
}

func (c *Camera) Scroll(yOffset float64) {
	c.scale += float32(yOffset) * 0.05
	c.updateView()
}

func (c *Camera) Key(key glfw.Key, action glfw.Action) {
...
func (c *Camera) GetPosition() (float32, float32, float32) {
	return c.x, c.y, c.z
}
